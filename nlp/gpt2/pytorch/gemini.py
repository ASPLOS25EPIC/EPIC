...
def main():
    ...
    checkpoint_process = multiprocessing.Process(target=async_save_checkpoint, args=(queue, ))
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        with tqdm(total=len(train_dataloader),
            ...
            for step, batch in enumerate(active_dataloader):
                ...
                lr_scheduler.step()
                optimizer.zero_grad()
                print("step {} takes {}s".format(step, time.time() - begin_step))
                ...
                #     }, '/data/baseline/Bert_{}_{}_{}-{}_base.pth.tar'.format(args.compressor,args.compressor_ratio,epoch,step))
                #     end_base = time.time()
                ...
                if hvd.rank() == 0 and args.freq > 0 and step % args.freq == 0:
                    checkpoint_data = ({
                        ...
                        'state_dict': model.state_dict(),
                        'optimizer' : optimizer.state_dict()
                        },'/ramdisk/gemini/GPT_{}_{}_{}-{}.pth.tar'.format(args.compressor,args.compressor_ratio,epoch,step))
...
def async_save_checkpoint(queue):
    while True:
        data = queue.get()
        if data is None:
            break  
