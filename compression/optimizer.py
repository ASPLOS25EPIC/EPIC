...
from .utils import find_duplicates, get_comm, get_compressor, get_memory, get_config, check_not_compress, check_not_ef, get_check, _to_cpu, get_pack
# from compression.utils import load_ssh_config, set_ssh_connection, close_ssh_connection, transfer_checkpoint_to_remote
...
def save_process(queue,freq,compressor):
    def add_tensors(key):
        pack[key]['tensors'][0] += diff[key]["tensors"][0]
    num_threads = 2 * os.cpu_count()
    pack = {}
    print("packing freq = {}".format(freq))
            ...
            if compressor == 'topk':
                ...
                if i % freq == 0:
                    ...
                    with concurrent.futures.ThreadPoolExecutor(max_workers=num_threads) as executor:
                        executor.map(add_tensors, diff.keys())
                    end = time.time()
                    print("merge gradient time: {}".format(end - begin))
...
class _DistributedOptimizer(torch.optim.Optimizer):
    def __init__(self, params, named_parameters,
        ...
        self.pack = get_pack(self._comm_params)
        
        # self.remote = comm_params['remote']
        ...
        # use sub process to save ckpt
        if hvd.rank() == 0:
            self.queue = multiprocessing.Queue()
            self.save_process = multiprocessing.Process(target=save_process, args=(self.queue,self.pack,comm_params.get('compressor', 'none')))
    ...
    def step(self, closure=None):
        ...
        # observation
        # if hvd.rank() == 0:
        #     print("compress time: {}".format(self.compress_time))
        # self.compress_time = 0
    ...
    def allreduce_receive(self, handles, ctx, name, tensor):
        ...
        if ctx == None:
            ...
            # self.diff_dict[name] = {'ctx':None,'tensors':output}
            return output
        else:
            self.diff_dict[name] = {'ctx':ctx, 'tensors': output}
            # self.diff_dict[name] = {'ctx':ctx, 'tensors': _to_cpu(output)}
            # print(name)
    ...
    def allgather_receive(self, result, ctx, name, tensor):
        ...
        if ctx == None:
            ...
            self.diff_dict[name] = {'ctx':None,'tensors':tensor_compressed}
        else:
            tensor_compressed = tensors_ag[0], tensors_ag[1]
            tensor_decompressed = self.compressor.decompress(tensor_compressed, ctx, name)
            self.diff_dict[name] = {'ctx':ctx, 'tensors': tensor_compressed}
            # self.diff_dict[name] = {'ctx':ctx, 'tensors': _to_cpu(tensor_compressed)}
            # print(name)
        return tensor_decompressed / self.world_size


    def save_differential_checkpoint(self, i, filename):        
        # naive save
        # begin = time.time()
        # torch.save(self.diff_dict, filename)
        # end = time.time()
        # print("saving time: {}".format(end - begin))
        
        # queue
        self.queue.put((self.diff_dict, filename, i))
        self.diff_dict = {}

    def subprocess_quit(self):
