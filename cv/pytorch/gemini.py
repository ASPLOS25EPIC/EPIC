...
def main():
    ...
    event.set()
    checkpoint_process = multiprocessing.Process(target=async_save_checkpoint, args=(queue, event))
    checkpoint_process.start()
    print("checkpoint process start")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        avg_train_time = train(train_loader, model, criterion, optimizer, epoch, args, queue, event)
    ...
    if hvd.rank() == 0:
        with open("/data/diff/time/{}_{}_{}_{}_time.txt".format(args.model, args.dataset, args.compressor,args.compressor_ratio), 'w') as f:
...
def train(train_loader, model, criterion, optimizer, epoch, args, queue, event):
                ...
                for j in range(0, len(images), args.batch_size):
                    ...
                    if hvd.rank() == 0 and args.freq > 0 and i % args.freq == 0:
                        checkpoint_data = ({
                            ...
                            },'/ramdisk/gemini/{}_{}_{}_{}_{}-{}.pth.tar'.format(args.model,args.dataset,args.compressor,args.compressor_ratio,epoch,i))
                        queue.put(checkpoint_data)
...
def async_save_checkpoint(queue, event):
    while True:
        data = queue.get()
        if data is None:
            break  
        ...
        begin = time.time()
        torch.save(full, filename)
        end = time.time()
        print("base checkpoint takes {}s".format(end - begin))
        event.set()  
