...
parser.add_argument('--model', default='resnet50',type=str, help='net type')
...
parser.add_argument("--comm_mode", default='allgather', type=str,
                    help='choose communication mode (allreduce,allgather) for compressor')
...
parser.add_argument("--freq", default=0, type=int,
                    help='how many iteration to save a whole checkpoint')
parser.add_argument("--remote", default='', type=str,
                    help='remote checkpoint host name')
parser.add_argument("--pack", default='1', type=int,
                    help='packing frequency')
...
def main():
    ...
    # CIFAR100
        ...
        train_dataset = \
            datasets.CIFAR100('/data/dataset/cv/cifar100/train',
                                train=True,
                                download=True,
                                transform=transforms.Compose([
    ...
    # CIFAR100
        ...
        val_dataset = \
            datasets.CIFAR100('/data/dataset/cv/cifar100/validation',
                                train=False,
                                download=True,
                                transform=transforms.Compose([
                                    #  transforms.Resize(256),
    ...
    if args.resume and hvd.rank() == 0:
        ...
        elif args.compressor == 'fp16':
            from compression.compressor.fp16 import FP16Compressor
            topk = FP16Compressor()
        if args.pack:
            model, optimizer = load_pack_differential_checkpoint(model,optimizer,train_loader_len,topk)
        else:
            model, optimizer = load_differential_checkpoint(model,optimizer,train_loader_len,topk)
        ckpt_end = time.time()
        print("checkpoint takes {}s".format(ckpt_end - ckpt_begin))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        avg_train_time = train(train_loader, model, criterion, optimizer, epoch, args, prev_model)
        ...
        #     }, '/data/delta/{}_{}_{}_{}_{}.pth.tar'.format(args.model,args.dataset,args.compressor,args.compressor_ratio,epoch))
        ...
        print("checkpoint takes {}s".format(dur_check))
        time_stat["checkpoint" + str(epoch)] = dur_check
    ...
    if hvd.rank() == 0:
    #     with open("/data/delta/time/{}_{}_{}_{}_time.txt".format(args.model, args.dataset, args.compressor,args.compressor_ratio), 'w') as f:
    #         for k, t in time_stat.items():
    #             print("Time stat {} : {}s".format(k, t))
    #             f.write("Time stat {} : {}s\n".format(k, t))
    #         f.write("Total time for all {} epochs = {}s\n".format(args.epochs-args.start_epoch, dur_full))
        print("Total time for all {} epochs = {}s".format(args.epochs-args.start_epoch, dur_full))


def train(train_loader, model, criterion, optimizer, epoch, args, prev_model=None):
        ...
        for i, data in enumerate(train_loader):
            if i < 100:
                ...
                for j in range(0, len(images), args.batch_size):
                    ...
                    # pack
                    # if hvd.rank() == 0 and args.diff :
                    #     optimizer.save_differential_checkpoint(i, '/data/delta/{}_{}_{}_{}_{}-{}_pack{}.pth.tar'.format(args.model,args.dataset,args.compressor,args.compressor_ratio,epoch,i,args.pack))

                    # if hvd.rank() == 0 and args.freq > 0 and i % args.freq == 0:
                    #     begin_base = time.time()
                    #     torch.save({
                    #         'epoch': epoch + 1,
                    #         'net': args.model,
                    #         'state_dict': model.state_dict(),
                    #         'best_acc1': 0,
                    #         'optimizer' : optimizer.state_dict(),
                    #     }, '/data/delta/{}_{}_{}_{}_{}_{}_base.pth.tar'.format(args.model,args.dataset,args.compressor,args.compressor_ratio,epoch,i))
                    #     end_base = time.time()
                    #     print("base checkpoint takes {}s".format(end_base - begin_base))
                    
                    if hvd.rank() == 0 and prev_model is not None and args.freq > 0 and i % args.freq == 0:
                        begin_base = time.time()
                        cur_model = copy.deepcopy(model)
                        diffcheck = calculate_parameter_difference(cur_model, prev_model)
                        torch.save(diffcheck, '/data/delta/{}_{}_{}_{}_{}_{}_base.pth.tar'.format(args.model,args.dataset,args.compressor,args.compressor_ratio,epoch,i))
                        prev_model = copy.deepcopy(model)
                        end_base = time.time()
                        print("base checkpoint takes {}s".format(end_base - begin_base))
...
def calculate_parameter_difference(cur_model, prev_model):
    diff = {}
    model1 = cur_model.half()
    model2 = prev_model.half()
    begin = time.time()
    for (name1, param1), (name2, param2) in zip(model1.named_parameters(), model2.named_parameters()):
        diff[name1] = param1- param2
    end = time.time()
    print("calculate parameter difference takes {}s".format(end - begin))
    return diff
...
def load_pack_differential_checkpoint(model, optimizer, interations, topk):
    filedir = '/data/delta/'
    named_parameters = list(model.named_parameters())
    _parameter_names = {v: k for k, v in sorted(named_parameters)}
    # skip synchronize
    # with optimizer.skip_synchronize():
    if args.compressor == 'topk':
        for i in range(args.pack-1, interations, args.pack):
            filepath = filedir + args.model + '_' + args.dataset + '_' + args.compressor + '_' + str(args.compressor_ratio) + '_' + args.resume + '-' + str(i) + '_pack' + str(args.pack) + '.pth.tar'
            tensor_compressed = torch.load(filepath)
            for j in range(0,args.pack):
                for key in tensor_compressed[i-args.pack+j+1].keys():  # the name for trainable params
                    tensor = topk.decompress(tensor_compressed[i-args.pack+j+1][key]['tensors'], tensor_compressed[i-args.pack+j+1][key]['ctx'], None)
                    for param_group in optimizer.param_groups:
                        for p in param_group['params']:
                            name = _parameter_names.get(p)
                            if(name == key):
        ...
        raise ValueError("No compressor found")
